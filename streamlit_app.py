...
@st.cache_data
def load_data():
    # Only load columns we actually use — saves memory on Streamlit Cloud
    use_cols = ['NACCAGE','SEX','RACE','MARISTAT','EDUC','NORMCOG','COGSTAT',
                'CDRSUM','CDRGLOB','NACCBMI','BPSYS','BPDIAS','HRATE','NACCNE4S',
                'DIABETES','HYPERTEN','DEP2YRS','NACCTBI','CANCER','CANCSITE','ALCOHOL',
                ...
                'MEMORY','ORIENT','JUDGMENT','ANIMALS','TRAILA','NACCGDS',
                'NACCADMD','NACCNCRD']
    try:
        all_cols = pd.read_csv("data.csv.gz", compression="gzip", nrows=0).columns.tolist()
        valid_cols = [c for c in use_cols if c in all_cols]
        df = pd.read_csv("data.csv.gz", compression="gzip", usecols=valid_cols)
    except Exception:
    ...
    # Cancer site categorization
    def categorize_cancer(site):
        if pd.isna(site) or str(site).strip() in ['0','','nan']:
            return 'None'
        site = str(site).lower().strip()
        if 'breast' in site: return 'Breast'
        if 'prostat' in site: return 'Prostate'
        if 'skin' in site or 'melanom' in site or 'basal' in site or 'squam' in site: return 'Skin'
        if 'colon' in site or 'colorect' in site or 'rectal' in site: return 'Colorectal'
        if 'lung' in site: return 'Lung'
        if 'bladder' in site: return 'Bladder'
        if 'thyroid' in site: return 'Thyroid'
        if 'lymph' in site or 'myelom' in site or 'leuk' in site: return 'Blood/Lymphoma'
        if 'kidney' in site or 'renal' in site: return 'Kidney'
        if 'uterus' in site or 'uteri' in site or 'cervi' in site or 'ovari' in site: return 'Gynecological'
        return 'Other'

    if 'CANCSITE' in df.columns:
        df['CancerSite'] = df['CANCSITE'].apply(categorize_cancer)
    else:
        df['CancerSite'] = 'None'
...
if cancer_filter == 'Yes':
    cancer_sites = sorted([s for s in df['CancerSite'].unique() if s != 'None'])
    cancer_site_filter = st.sidebar.selectbox("Cancer Site", ['All'] + cancer_sites)
else:
    cancer_site_filter = 'All'
...
if cancer_site_filter != 'All': dff = dff[dff['CancerSite'] == cancer_site_filter]
...
with tab_viz:
    ...
    with col1:
        ...
        fig1.update_layout(plot_bgcolor='#FFFFFF',paper_bgcolor='#F5F5F0',font=dict(color='#2D2D2D'),height=380)
    ...
    with col2:
        ...
        fig2.update_layout(plot_bgcolor='#FFFFFF',paper_bgcolor='#F5F5F0',font=dict(color='#2D2D2D'),height=380,coloraxis_showscale=False)
    ...
    with col3:
        ...
        fig3.update_layout(plot_bgcolor='#FFFFFF',paper_bgcolor='#F5F5F0',font=dict(color='#2D2D2D'),height=380)
    ...
    with col4:
        ...
        fig4.update_layout(plot_bgcolor='#FFFFFF',paper_bgcolor='#F5F5F0',font=dict(color='#2D2D2D'),height=380)
    ...
    with col5:
        ...
        fig5.update_layout(plot_bgcolor='#FFFFFF',paper_bgcolor='#F5F5F0',font=dict(color='#2D2D2D'),height=380)
    ...
    with col6:
        ...
        fig6.update_layout(plot_bgcolor='#FFFFFF',paper_bgcolor='#F5F5F0',font=dict(color='#2D2D2D'),height=380)
...
with tab_predict:
    if not model_loaded:
        ...
        with col_left:
            ...
            if p_cancer[1] == 1:
                cancer_site_options = ['Breast','Prostate','Skin','Colorectal','Lung','Bladder',
                                       'Thyroid','Blood/Lymphoma','Kidney','Gynecological','Other']
                p_cancer_site = st.selectbox("Cancer Site", cancer_site_options)
            else:
                p_cancer_site = 'None'
            ...
            # --- PREDICTION MODE ---
            st.markdown(f"#### Prediction Mode")
            pred_mode = st.radio("", ['Balanced', 'Screening (high sensitivity)'], horizontal=True)
        ...
        with col_right:
            if predict_btn:
                ...
                threshold = 0.3 if pred_mode == 'Screening (high sensitivity)' else 0.5
                pred_class = 1 if prob_normal >= threshold else 0
                ...
                <div style="background:white;border:2px solid {tc};border-radius:12px;padding:30px;text-align:center;margin-bottom:20px;">
                    ...
                    <p style="color:#6B6B6B;font-size:13px;margin:5px 0;">
                        Prediction: <strong style="color:{UTRGV_GREEN if pred_class==1 else '#D32F2F'}">
                        {'Normal Cognition' if pred_class==1 else 'Cognitive Impairment'}</strong>
                        <span style="color:#999;font-size:11px;">(threshold: {threshold:.2f} | {pred_mode})</span>
                    </p>
                </div>
                """, unsafe_allow_html=True)
                ...
                try:
                    ...
                    fig_shap.update_layout(title="What's Driving This Prediction?",
                        ...
                        plot_bgcolor='#FFFFFF',paper_bgcolor='#F5F5F0',font=dict(color='#2D2D2D'),height=480,
                ...
                summary_data = {
                    ...
                    'Cancer Site':p_cancer_site if p_cancer[1]==1 else 'N/A',
