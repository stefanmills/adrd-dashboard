...
<style>
    ...
    .stApp {{background-color: #F5F5F0; color: #2D2D2D;}}
    header[data-testid="stHeader"] {{background-color: {UTRGV_GREEN};}}
    
    /* All text dark */
    .stApp p, .stApp span, .stApp label, .stApp div {{color: #2D2D2D;}}
    .stMarkdown p {{color: #2D2D2D;}}
    h1, h2, h3, h4 {{color: #2D2D2D !important;}}
    
    /* Sidebar */
    section[data-testid="stSidebar"] {{background-color: #FFFFFF;}}
    section[data-testid="stSidebar"] p, section[data-testid="stSidebar"] label,
    section[data-testid="stSidebar"] span {{color: #2D2D2D;}}
    section[data-testid="stSidebar"] h1, section[data-testid="stSidebar"] h2,
    section[data-testid="stSidebar"] h3 {{color: {UTRGV_GREEN} !important;}}
    ...
    .stTabs [data-baseweb="tab"] {{
        ...
        padding: 12px 24px; color: #2D2D2D; font-weight: 600;
        flex: 1; text-align: center;
        border: 1px solid #E0E0E0;
    }}
    .stTabs [aria-selected="true"] {{
        background-color: {UTRGV_GREEN}; color: white !important;
        border: 1px solid {UTRGV_GREEN};
    }}
    .stTabs [data-baseweb="tab-panel"] {{color: #2D2D2D;}}
    ...
    div[data-testid="metric-container"] label {{color: #6B6B6B !important;}}
    div[data-testid="metric-container"] div[data-testid="stMetricValue"] {{color: #2D2D2D !important;}}
    
    /* Selectbox and inputs */
    .stSelectbox label, .stSlider label, .stNumberInput label {{color: #2D2D2D !important;}}
    .stSelectbox div[data-baseweb="select"] {{color: #2D2D2D;}}
    
    /* Data table */
    .stDataFrame {{color: #2D2D2D;}}
    
    /* Reduce top padding */
    .block-container {{padding-top: 1rem;}}
    
    /* Plotly charts on white */
...
with tab_predict:
    if not model_loaded:
        ...
        with col_left:
            ...
            # --- PREDICTION MODE ---
            st.markdown(f"#### ⚙️ Prediction Mode")
            pred_mode = st.radio("", ['Balanced', 'Screening (high sensitivity)'], horizontal=True)

            predict_btn = st.button("🔮 Predict Cognitive Status", type="primary", width="stretch")
